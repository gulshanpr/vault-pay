...
contract VaultAdapter is ReentrancyGuard, Ownable {
    ...
    function settle(
        ...
        MerchantRegistry.MerchantConfig memory cfg = REGISTRY.getConfig(
            merchant
        );
        ...
        if (
            cfg.mode == MerchantRegistry.PayoutMode.SHARES_ONLY ||
            cfg.mode == MerchantRegistry.PayoutMode.SPLIT
        ) {
        ...
        if (cfg.mode == MerchantRegistry.PayoutMode.USDC_ONLY) {
            ...
            sharesMinted = IMetaMorpho(vault).deposit(
                toShares,
                cfg.merchantPayout
            );
    ...
    function redeemAllShares(
        ...
        assetsReceived = vaultContract.redeem(
            userShares,
            recipient,
            address(this)
        );
